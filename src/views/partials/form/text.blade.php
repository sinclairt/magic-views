<div class="form-group @if($errors->has($field)) has-error @endif">
    ...
    <div class="col-sm-6">
        <input type="text" class="form-control" id="{{ $field }}" placeholder="{{ trans('magic-views::magic-views.fields.' . $modelName . '.' .  $field . '_placeholder') }}" name="{{ $field }}" value="{{ old($field, isset($model) ? $model->$field : null) }}">
    </div>
    @if($errors->has($field))
        <div class="col-sm-3">
